...
function CsMainCont() {
  ...
  useEffect(() => {
    const fetchCSStudents = async () => {
      try {
        const response = await axios.get('/api/cs-students');
        setStudents(response.data);
      } catch (error) {
        console.error('Error fetching COMPUTER SCIENCE students:', error);
        setError('Failed to fetch COMPUTER SCIENCE students. Please try again later.');
      }
    };
    ...
    <main className="w-full ml-5">
      ...
      <div className="flex flex-row">
        <div className="row-span-3 col-span-4 items-center bg-white rounded-xl shadow-lg px-6 py-4 mt-[140px] mr-8 flex-grow">
          <p className="text-3xl mt-3 font-extrabold font-poppins text-blue-800">Computer Science Enrollees</p>
          <p className="mt-3 mb-3 text-base font-semibold text-blue-800">List of enrolled Computer Science students</p>
        </div>
      </div>
